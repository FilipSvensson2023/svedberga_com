    ...
    nav_main,
		...
		Default = {
            ...
            window.assignIndex = function(em) {
                ...
                if(em.getElementsByClassName('l4ml').length){
...
function checkIfExists() {
    ...
    if (paginationContainer.length) {
        ...
        var pElement = paginationContainer.find('a');
...
// checkLenght =  () => {
//    let countt =  $('.product-list-table-design tr').length;
//     $('.short_pro .count_pp').text(countt -1)
// }
...
// add to cart extra
async function handleAddToCartClick (clickedButton, unitqty, handle) {
    var parentTr = $(clickedButton).closest('tr');
    var targetButton = parentTr.find('.Quantity-heading button[type="submit"]');

  var target_form = parentTr.find('.Quantity-heading form');

 await jQuery.getJSON(`/products/${handle}.js`, function(product) {
      console.log();

target_form.find('.quen-cart input[type="hidden"]').val(product.variants[0].id)
    } );


  var form_id = target_form.attr('id');

  console.log(form_id);

 var current_val = targetButton.prev('.input-amount').find('[quantity_btn]').val();

  result = current_val * unitqty;

   targetButton.prev('.input-amount').find('[quantity_btn]').val(result);
...
var formData = $("#"+form_id).serialize();
        ...
        $.ajax({
            ...
            success: function(response) {
              ...
              targetButton.prev('.input-amount').find('[quantity_btn]').val(current_val);
                ajaxCart.load(); 

              // $.ajax({ 
              //     url: 'https://svedbergaplantskola.se/?section_id=side-cart',
              //     type: 'GET',
              //     // data: {section_id:'side-cart'},
              //     success: function(response){
              //         console.log('response is next', response); 
              //       ajaxCart.load();  
              //      setTimeout(function(){ 
              //       $('#cart').empty(); 
              //        $('#cart').html(response);
...
// Last viewed functionality with content change 
$(document).ready(function(){
    function checkURL() {
        var url = window.location.href;
        if (url.includes('products')) {
            sessionStorage.setItem('currentProductURL', url);
            // Update last viewed product URL if it's the first time visiting a product page
            if (!sessionStorage.getItem('lastViewedProductURL')) {
                sessionStorage.setItem('lastViewedProductURL', url);
            }
        }
    }

    // Check if the user has visited a product page before
    if (sessionStorage.getItem('lastViewedProductURL')) {
        $('#last_viewed').text('Last Viewed'); 
         $('#last_viewed').addClass("last_viewed");
    }
    ...
    // Redirect to the last viewed product URL when button is clicked
    $('#last_viewed').click(function(){
        var lastViewedURL = sessionStorage.getItem('lastViewedProductURL');
        if (lastViewedURL) {
            window.location.href = lastViewedURL;
        } else {
            alert('No last viewed product URL saved.');
        }
    });
}); 

// fixed pagination bug button creating when there's no products 

function hidePaginationIfNoProducts() {
    var text_val = $('#total_prds').text();
    if (text_val.trim() === 'No products found') {
        $('.pagination-item').css('display', 'none');
    }
}

setInterval(hidePaginationIfNoProducts, 100);  


// inventory bug fix on add to cart 
$('.add-to-cart').click(function() {
  ...
  $.ajax({
    ...
    success: function(cartData) {
      ...
      // Update the inventory_span element with the message and show it
  ...
  // handleAddToCartClick(button, '{{ product.metafields.global.unit_qty }}', '{{ product.handle }}');
});
...
//             var updatedCount = totalCount > 1 ? totalCount - 1 : 0; 
//             setTimeout(function(){
//                  $("#cart-count").text(totalCount);
//             console.log("Total count of items in the cart: " + updatedCount);
