...
var ajaxCart = (function(module) {
  ...
  mainProductAddToCart = function(form, listItem) {
    ...
    var count = item.item_count;
        ...
        var total = item.total_price;
        total = total/100;

        total = total.toFixed(0);


$('.l4tt.m15.form-group').find('li').eq(0).html(`<span>Totala produkter  (${count})</span> ${total} kr`);
$('.l4tt.m15.form-group').find('li').eq(1).html(`<span>Total ink moms</span> ${total} kr`);
	...
	updateCartPanel = function(response = false, openCartPanel = true, undoRemove = false, forceRefetch = true) {
		if (response) {
	...
	handleCartUpdates = function(container) {
		var updateItemInput = container.querySelectorAll('input[name="updates[]"]'),
          //custom code
          updateItemInputMeta = container.querySelectorAll('input[name="updatesmeta[]"]');
      //custom code ends
      ...
      //custom code
        Array.from(updateItemInputMeta).forEach(function (input) {
        input.addEventListener('change', function (e) {
            updateItemQtyMeta(e.target, container);
        });
    });
//custom code ends
  ...
  //custom code
      	updateItemQtyMeta = function(input, container) {
         console.log("updae--");
         console.log(input.dataset.metaquentity);
     
		line = parseInt(input.dataset.line),
		quantity = parseInt(input.value)*parseInt(input.dataset.metaquentity) ;
		config = {
			method: 'POST',
			body: JSON.stringify({
					'line': line,
